...
if ( urlParams.has('VR') ) {
    var VR = true;
    var y_offset = 2; // vertical offset to get camera at right height
}
else {
    var VR = false;
    var y_offset = 0;
...
add_temp_axes();
if ( VR ) {
    var gui = GUIVR.create('Parameters');
    add_gui();
    gui.position.x = 2;
    gui.position.z = -2;
...
function add_gui() {
    ...
    gui.add( params, 'H_stack').min(0.1).max(100).step(0.1).listen().name('Stack Height (m)').onChange( function() { stack.scale.y = params.H_stack;
...
function moveParticles( dt ) {
    ...
    if ( params.adiab ) {
        for (var i = 0; i < N_max; i++) {
            ...
            if ( params.colour_by === 'Relative Temp' ) { var c = lut.getColor(T[i] - T_elevation);}
            else if ( params.colour_by === 'Temp' ) { var c = lut.getColor(T[i]); }
            else if ( params.colour_by === 'Vertical velocity' ) { var c = lut.getColor(v_y[i]); }
